...
def extract_data_from_pdf(pdf_path):
    ...
    invoice_pattern = r'\b[A-Z]{3,4}\d-\d{4,7}\b'
    invoice_pattern_2 = r'\b[A-Z]{2,4}-\d{3,6}\b'
    ...
    sale_bill_pattern = r'₹\d+,\d+\.\d+(?=\s*\n|$)'  # Pattern to extract sale bill amounts
    net_amount_pattern = r'₹\d+,\d+\.\d+(?=\s+\d+%?\s(?:CGST|SGST|IGST)|TOTAL:)'
    ...
    for page_num in range(pdf_document.page_count):
        ...
        invoices_2 = re.findall(invoice_pattern_2, text)
        ...
        # Process each entry
        for i in range(max(len(invoices), len(net_amounts), len(invoices_2))):  # Iterate based on the maximum number of matches
            order_date = order_dates[i] if i < len(order_dates) else ''
            order_date_split = order_date.split('.')
            month = ''
            if len(order_date_split) == 3:  # Ensure there are three parts (day, month, year)
            ...
            data.append({
                ...
                'Invoice No 2': invoices_2[i] if i < len(invoices_2) else '',  # Add second invoice number column
                'Month': month,
